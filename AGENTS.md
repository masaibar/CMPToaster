...
- `toast/src/androidHostTest/kotlin/...` contains JVM/Robolectric host tests.
...
- `./gradlew :toast:testAndroidHostTest` runs Android host tests with JUnit and Robolectric.
...
Use Kotlin official style with 2-space indentation, matching the existing Gradle and Kotlin files. Keep APIs minimal and platform behavior behind `expect`/`actual` declarations. Public types use `PascalCase`; functions, properties, and test methods use `camelCase`. Keep package paths under `io.github.masaibar.toastcmp`.
